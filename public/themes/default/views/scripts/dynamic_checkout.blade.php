...
// console.log(shop); modify by ari 06012021
			if(! shop ){
				disableCartCheckout(cart);
			}
			else{
				if($(".summary-shipping"+cart).text()==""||number($(".summary-shipping"+cart).text())==0){
				disableCartPayment('{{ trans('theme.notify.shipping_cost_not_emty') }}');
				$("#payment-midtranslist").attr("disabled",true);
			}
...
if($(".payment-option").on("ifChecked",function(){
	if(this.value=="midtrans"){
		$("#pay-now-btn").hide();
		$("#btncheckoutmidtrans").show();
	}else{
		$("#pay-now-btn").show();
		$("#btncheckoutmidtrans").hide();
	}
}))
			...
			$('.customer-address-list .address-list-item').on('click', function(){
				...
				// checkShippingZone(radio.data('country'), radio.data('state'));
			});
        ...
        //     }, 100);
        // });
		$('a.dynamic-shipping-rates').hover(function() {
    var a = $(this);
    var contentcourier = "";
    var cart = $(this).data('cart');
    var address = $(".ship-to-address:checked").val();
    console.log(address);
    if (address === undefined) {
        var options = '<p class="space10"><span class="space10"></span>Please select destination address</p>';
        contentcourier = '<div class="popover-form" id="shipping-options-popover" data-cart="' + cart + '">' +
            options + '</div>';
        a.popover({
            html: true,
            placement: 'bottom',
            content: contentcourier
        }).on('mouseenter', function() {
            var _this = this;
            $(this).popover('show');
            $('.popover').on('mouseleave', function() {
        ...
        $.ajax({
            url: "{{ route('ajax.getShippingCourier') }}",
            type: 'GET',
            data: {
                'cart': cart,
                'address_id': address
            ...
            complete: function(xhr) {
                ...
                if (xhr.status == 200) {
                    ...
                    if (courier.length > 0) {
                        var options = '<table class="table table-striped" id="checkout-options-table">';
                        courier.forEach(function(item) {
                            // preChecked = String(current) == String(item.name) ? 'checked' : '';
                            // var shippingRate = Number(item.rate) + Number(handlingCost);

                            options += '<tr><td><div class="radio"><label id="' + item.id + '"><input type="radio" name="shipping_option" id="' + item.courier + "-" + item.typeCourier + '" value="' + item.cost + '" ' + preChecked + '>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' + item.courier + '</label></div></td>' +
                                '<td>' + item.typeCourier + '</td>' +
                                '<td><small class"text-muted">' + item.est + ' days</small></td>' +
                                '<td><span>{{ get_currency_prefix() }}' + getFormatedValue(item.cost) + '{{ get_currency_suffix() }}</span></td></tr>';
                        });
                        options += '</table>';
                        contentcourier = '<div class="popover-form" id="shipping-options-popover" data-cart="' + cart + '">' +
...
$("#btncheckoutmidtrans").on("click",function(e){
		e.preventDefault();
		var datasend= new FormData();
		datasend=$("#checkoutForm").serialize();
		var cart=$("#listcart").val();
		var ur = "{{ route('order.orderbymidtrans', ['cart' =>  ':cart']) }}";
		    var url =ur.replace(':cart', cart);
		var valid=true;
		var shippingcost=$("input[name='shipping_cost'").val();
		if(shippingcost==0){
			@include('theme::layouts.notification', ['message' => trans('theme.notify.shipping_cost_not_emty'), 'type' => 'danger', 'icon' => 'times-circle'])
			valid=false
		}
		if(valid){
		...
		function calculateCartTotal(cart)
		{
      	...
      	function calculateOrderSummary(cart)
      	{
			  ...
			  var total = getOrderTotal(cart);
						...
						console.log(grands);
			  	$("#summary-grand-totals"+cart).text(getFormatedValue(grands));
		...
		function getCartWeight(cart)
		{
      	...
      	function getTotalAmount(cart)
      	{
	        ...
	        var shipping  =Number($('#summary-shipping'+cart).text());//  getShipping(cart);
	        var discount  = getDiscount(cart);
		...
		{
			var filtered = getShippingOptions(cart);console.log(filtered);
		...
		function setShippingCost(cart, name = '', value = 0, id = '')
		{
			...
			$("#courier_id"+cart).val(id);
			$("#shipping-cost"+cart).val(value);
			calculateTax(cart);
			enableCartPayment();	
			if($(".payment-option:checked").val()!=undefined && $(".payment-option:checked").val()=='midtrans'){
				$("#pay-now-btn").hide();
		$("#btncheckoutmidtrans").show();
			}
		
			$("#payment-midtranslist").attr("disabled",false);
			return;
		}
		...
		function resetDiscount(cart)
		{
		  ...
		  function getShopCourier(cart,address){
			var courier;
			$.ajax({
			    url: '{{ route('ajax.getShippingCourier') }}',
			    type: 'GET',
			    data: {'cart':cart,'address_id':address},
			    dataType: 'JSON',
			    complete: function (xhr, textStatus) {
			    	courier =xhr.responseJSON;
			    },
			});
			return courier;
		  }
	...
	function getPaymentMidstransurl(){
			  var datasend= new FormData();
			  datasend= $("#checkoutForm").serialize();
			
			$.ajax({
			    url: '{{ route('ajax.getPaymidtransUrl') }}',
			    type: 'POST',
			    data: datasend,
			    dataType: 'JSON',
			    complete: function (xhr, textStatus) {
