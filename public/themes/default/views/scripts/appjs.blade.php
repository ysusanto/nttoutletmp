...
function getTrackShipping(orderId){
    // var orderid=$("#orderIdshipp").val();
			var action = "{{ route('order.trackship') }}";
...
function btnfinishorder(orderId){
    // var orderid=$("#orderIdshipp").val();
			var action = "{{ route('order.finishorder') }}";
	      	// var data = $( this ).serialize();
	      	$.ajax({
		        url: action,
		        type: 'POST',
