...
class ListHelper
{
    ...
    public static function popular_items($days = 7, $count = 15)
    {
        ...
        ->zipcode()->groupBy('product_id', 'shop_id')->limit($count)->get();
   ...
   /* public static function popular_items($days = 7, $count = 15)
    ...
    public static function featured_items()
    {
       ...
       return \DB::table('inventories')->whereIn('id', $items)->orderBy('title', 'asc')->pluck('title', 'id')->toArray();
    }
     ...
     * @return array
     */
    public static function best_find_under($price)
    {
    ...
    /**
     * Get Trending now categories
     * @return array
     */
    public static function trending_now_categories()
    {
        if ($trending_categories = get_from_option_table('trending_categories', [])){
            return Category::whereIn('id', $trending_categories)->get();
        }
     ...
     * @return array
     */
    public static function payment_methods($exclude_manual_types = false)
    {
        $query = \DB::table('payment_methods')->where('enabled', 1);

        if ($exclude_manual_types) {
            $query->where('type', '!=', PaymentMethod::TYPE_MANUAL);
        }
    ...
    public static function getTypeRo($type)
    {
        $result = "";
