...
<script language="javascript" type="text/javascript">
  ...
  (function($, window, document) {
      ...
      if (payment_methods.length == 0) {
        $("#global-alert-msg").html('{!! trans('
          messages.notice.no_active_payment_method ') . '
          ' . ' < a href = "' . route('admin.setting.config.paymentMethod.index') . '" > ' . trans('
          app.activate ') . ' < /a>' !!}');
        ...
        if (billing_address.length == 0) {
          $("#global-alert-msg").html('{!! trans('
            messages.notice.no_billing_address ') . '
            ' . ' < a class = "ajax-modal-btn btn btn-new"
            href = "javascript:void(0)"
            data - link = "' . route('address.create', ['customer', $customer->id]) . '" > < i class = "fa fa-plus-square-o" > < /i>' . trans('app.add_address') . '</a > ' !!}');
          $("#global-alert-box").removeClass('hidden');
        }
        ...
        if (cart) {
          setPackagingCost('{{$default_packaging->name}}', {
            {
              $default_packaging - > cost
            }
          }, {
            {
              $default_packaging - > id
            }
          });
          calculateOrderTotal();
        }
        ...
        if (!shipping_options) {
          $("#global-alert-msg").html('{{ trans('
            messages.notice.no_shipping_option_for_the_zone ') }}');
          $("#global-alert-box").removeClass('hidden');
        } else if ($.isEmptyObject(shipping_options)) {
          $("#global-alert-msg").html('{!! trans('
            messages.notice.no_rate_for_the_shipping_zone ', ['
            zone ' => optional($shipping_zone)->name]) !!}');
          $("#global-alert-box").removeClass('hidden');
        }

        var apply_btn = '<div class="spacer5"></div><button class="popover-submit-btn btn btn-flat btn-new btn-lg btn-block" type="button">{{ trans('
        app.apply ') }}</button>';
        ...
        $(document).on("click", ".popover-submit-btn", function() {
          ...
          switch (nodeId) {
            case 'shipping-options-popover':
              ...
              var name = shipping.attr('id') == 'custom_shipping' ? '{{ trans('
              app.custom_shipping ') }}': shipping.attr('id');
        ...
        $('a.discount-options').popover({
          ...
          content: function() {
            ...
            var options = '<div class="input-group" id="discount-popover"><span class="input-group-addon">{{ config('
            system_settings.currency_symbol ') ?: '
            $ ' }}</span><input id="input-discount" name="discount" class="form-control" value="' + current + '" type="number" step="any" placeholder = {{ trans('
            app.discount ') }}></div>';
        ...
        $('a.dynamic-shipping-rates').popover({
          ...
          content: function() {
            ...
            var preChecked = String(current) == '{{ trans('
            app.custom_shipping ') }}' ? 'checked' : '';
            var custValue = preChecked == 'checked' ? getShipping() : '';

            var custom_shipping = '<div class="radio"><label id=""><input type="radio" name="shipping_option" id="custom_shipping"' + preChecked + '>{{ trans('
            app.custom_shipping ') }}</label></div>' +
              '<div class="input-group"><span class="input-group-addon">{{ config('
            system_settings.currency_symbol ') ?: '
            $ ' }}</span><input id="input-shipping" name="custom_shipping" class="form-control" value="' + custValue + '" type="number" step="any" placeholder = {{ trans('
            app.placeholder.custom_shipping ') }}></div>';
        ...
        $('#add-to-cart-btn').click(
          function() {
            ...
            var node = '<tr id="' + ID + '">' +
              '<td><img src="' + imgSrc + '" class="img-circle img-md" alt="{{ trans('
            app.image ') }}"></td>' +
              ...
              '<input name="cart[' + numOfRows + '][unit_price]" value="' + productObj[ID].salePrice + '" id="price-' + ID + '" type="number" class="form-control itemPrice no-border" placeholder="{{ trans('
            app.price ') }}" required>' +
              '</div>' +
              '<td>x</td>' +
              '<td class="nopadding-right" width="10%">' +
              '<input name="cart[' + numOfRows + '][quantity]" value="1" type="number" id="qtt-' + ID + '" class="form-control itemQtt no-border" placeholder="{{ trans('
            app.quantity ') }}" required>' +
              '</td>' +
              ...
              '</span>' +
              '</td>' +
              '<td class="small"><i class="fa fa-trash text-muted deleteThisRow" data-toggle="tooltip" data-placement="left" title="{{ trans('
            help.romove_this_cart_item ') }}"></i></td>' +
              '</tr>';
        ...
        function calculateOrderTotal() {
          ...
          if (!cartWeight) {
            $("#global-alert-msg").html('{{ trans('
              messages.notice.cant_cal_weight_shipping_rate ') }}');
            $("#global-alert-box").removeClass('hidden');
          }
        ...
        function calculateOrderSummary() {
          ...
          var grands = (total - discount) + getTax(cart);
          console.log(grands);
          $("#summary-grand-total-cart" + cart).text(getFormatedValue(grands));
