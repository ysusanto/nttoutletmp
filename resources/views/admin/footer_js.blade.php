<script type="text/javascript">
	...
	**************************************/
	...
	function initMassActions()
	{
		...
		$(".trackaction").hover(function(e){
			var a=$(this);
			var orderid=$("#orderIdshipp").val();
			var action = "{{ route('admin.order.order.getTrackshipping') }}";
	      	// var data = $( this ).serialize();
	      	$.ajax({
		        url: action,
		        type: 'GET',
		     data:"id="+orderid,
		            ...
		            if (xhr == 200){
					 ...
					 datatrack.forEach(function(item){
						 if(x==0){
							...
							html +="<div class='col-md-8'><span style='font-weight:bold;color:"+color+"'>"+item.date_output+"</span></div>";
							html +="<div class='col-md-4 float-right'>"+item.time+" wib</div>";
							html +="</div> <div class='row'>";
							html +="<div class='col-md-8'>"+item.desc+"</div>";
							html +="</div></li> ";
							x++;
					 })
		        ...
		        error: function (xhr,textStatus) {
                    console.log(xhr);
                    $("#divlistrack").html("<p style='padding:10px;font-weight:bold'>"+xhr.responseText+"</p>");
                    $("#listTrackingmodal").modal("show");
